...
To create your new lab report, copy and paste the raw Markdown report from the Github page. **For this lab in particular, it will be critical that you follow along with this code (in place of making your own lab file)**
...
You can also use the `inference` command to compare the difference of two population proportions. The code below compares the atheism rates between two countries, the US and Sweden. In this example, Sweden (first alphabetically) is used as the first population proportion. 


```{r us-sweden atheism}
us.swe12 <- atheism %>%
  filter(nationality == "United States"|nationality == "Sweden", year == "2012") %>% 
  mutate(nationality = factor(nationality))

inference(y = response, x = nationality, data = us.swe12, statistic = "proportion",
          type = "ci", method = "theoretical", success = "atheist")


inference(y = response, x = nationality, data = us.swe12, statistic = "proportion",
          type = "ht", alternative = "twosided", method = "theoretical", success = "atheist")
```

8. Interpret the confidence interval shown in the code above, and identify the components of the plots that show up when running the inference command using a hypothesis test (`type = "ht"`).
...
9.  Describe the relationship between `p` and `me`. Include the margin of
...
10.  Describe the sampling distribution of sample proportions at $n = 300$ and 
    $p = 0.1$. Be sure to note the center, spread, and shape.

11.  Keep $n$ constant and change $p$. How does the shape, center, and spread 
...
12.  Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?

13.  If you refer to Table 6, you'll find that Australia has a sample 
...
14.   Is there convincing evidence that Spain has seen a change in its atheism index 
...
15.   Is there convincing evidence that the US has seen a change in its atheism index 
    between 2005 and 2012? As always, write out the hypotheses for any tests you 
    conduct and outline the status of the conditions for inference. If you find a 
    significant difference, also quantify this difference with a confidence interval.

16.   If in fact there has been no change in the atheism index in the countries 
...
17.   Suppose you're hired by the local government to estimate the proportion of 
